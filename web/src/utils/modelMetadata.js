...
// 该依赖（@opencode-ai/models）在部分部署环境的 node_modules 中缺失，
// 用变量说明符 + @vite-ignore 避免 Vite 在编译期解析裸模块导致整页编译失败，
// 运行期加载失败时优雅降级为空目录。依赖声明仍保留在 package.json 中。
const MODEL_CATALOG_SPECIFIER = '@opencode-ai/models/snapshot'

export const USD_TO_CNY_RATE = 7

export const loadModelMetadataCatalog = () => {
  if (!catalogPromise) {
    catalogPromise = import(/* @vite-ignore */ MODEL_CATALOG_SPECIFIER)
