...
async def test_confirm_tmp_thread_attachments_keeps_duplicate_names_separate(monkeypatch, tmp_path: Path):
    ...
    first_upload = tmp_path / "threads" / "thread-1" / "user-data" / "uploads" / Path(first["original_path"]).name
    second_upload = tmp_path / "threads" / "thread-1" / "user-data" / "uploads" / Path(second["original_path"]).name
    assert first_upload.read_bytes() == b"first"
    assert second_upload.read_bytes() == b"second"
