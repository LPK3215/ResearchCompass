...
class EvaluationRepository:
    ...
    async def create_experiment_with_variants(
        self, data: dict[str, Any], variants_data: list[dict[str, Any]]
    ) -> EvaluationExperiment:
        experiment = EvaluationExperiment(**data)
        variants = [EvaluationExperimentVariant(**item) for item in variants_data]
        async with pg_manager.get_async_session_context() as session:
            session.add(experiment)
            session.add_all(variants)
        return experiment

    async def get_experiment(self, experiment_id: str) -> EvaluationExperiment | None:
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperiment).where(EvaluationExperiment.experiment_id == experiment_id)
            )
            return result.scalar_one_or_none()

    async def list_experiments(self, kb_id: str) -> list[EvaluationExperiment]:
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperiment)
                .where(EvaluationExperiment.source_kb_id == kb_id)
                .order_by(EvaluationExperiment.created_at.desc())
            )
            return list(result.scalars().all())

    async def list_recoverable_experiments(self) -> list[EvaluationExperiment]:
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperiment).where(EvaluationExperiment.status.in_(["queued", "running"]))
            )
            return list(result.scalars().all())
        ...
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperimentVariant).where(
                    ...
                    EvaluationExperimentVariant.status == "running",
                )
            )
            for record in result.scalars().all():
                record.status = "queued"

    async def update_experiment(self, experiment_id: str, data: dict[str, Any]) -> EvaluationExperiment | None:
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperiment).where(EvaluationExperiment.experiment_id == experiment_id)
            )
    ...
    async def list_experiment_variants(self, experiment_id: str) -> list[EvaluationExperimentVariant]:
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperimentVariant)
                .where(EvaluationExperimentVariant.experiment_id == experiment_id)
                .order_by(EvaluationExperimentVariant.variant_index.asc())
            )
            return list(result.scalars().all())

    async def get_experiment_variant(self, variant_id: str) -> EvaluationExperimentVariant | None:
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperimentVariant).where(EvaluationExperimentVariant.variant_id == variant_id)
            )
            return result.scalar_one_or_none()

    async def update_experiment_variant(
        self, variant_id: str, data: dict[str, Any]
    ) -> EvaluationExperimentVariant | None:
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                select(EvaluationExperimentVariant).where(EvaluationExperimentVariant.variant_id == variant_id)
            )
