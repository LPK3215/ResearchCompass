...
class KnowledgeChunkRepository:
    ...
    async def list_academic_by_file_id(
        ...
        *,
        ...
        if section_type:
            filters.append(KnowledgeChunk.chunk_metadata["section_type"].as_string() == section_type)
        if chunk_id:
        ...
        async with pg_manager.get_async_session_context() as session:
            total_result = await session.execute(select(func.count()).select_from(KnowledgeChunk).where(*filters))
            result = await session.execute(
                select(KnowledgeChunk)
                .where(*filters)
                .order_by(KnowledgeChunk.chunk_index.asc())
