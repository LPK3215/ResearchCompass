...
class EvaluationMetricsCalculator:
    ...
    def calculate_retrieval_metrics(
        ...
        # 同一语料的不同分块策略不会产生相同 chunk_id。跨索引对照时使用
        # 源文件内容哈希作为检索身份，仍保留 chunk_id 作为常规评估的默认值。
        retrieved_ids = []
        for chunk in retrieved_chunks:
            metadata = chunk.get("metadata", {})
            identity = chunk.get(identity_field) or metadata.get(identity_field)
            retrieved_ids.append(str(identity) if identity else "")
