...
def _build_chunk_records(
    text_chunks: list[str | dict[str, Any]], file_id: str, filename: str, source_text: str | None = None
) -> list[dict[str, Any]]:
    ...
    for idx, chunk_value in enumerate(text_chunks):
        ...
        if isinstance(chunk_value, dict):
            chunk_content = chunk_value.get("content")
            metadata_value = chunk_value.get("metadata")
            chunk_metadata = metadata_value if isinstance(metadata_value, dict) else None
            source_start = chunk_value.get("source_start")
            source_end = chunk_value.get("source_end")
        else:
            chunk_content = chunk_value
        ...
        if isinstance(source_start, int) and isinstance(source_end, int):
            start_char_pos = source_start
            end_char_pos = source_end
...
def _dispatch_markdown_parser(
    ...
    if parser_id == "academic":
        return academic.chunk_markdown(filename, markdown_content, parser_config)
