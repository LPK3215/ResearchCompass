...
class MilvusKB(KnowledgeBase):
    ...
    async def _hydrate_chunk_sources(self, kb_id: str, chunks: list[dict]) -> None:
        ...
        file_records = await KnowledgeFileRepository().list_by_file_ids(file_ids)
        content_hash_by_file_id = {
            str(record.file_id): str(record.content_hash or "") for record in file_records if record.content_hash
        }
        records_by_chunk_id = {record.chunk_id: record for record in chunk_records}
        for chunk in chunks:
            metadata = chunk.get("metadata")
            if not isinstance(metadata, dict):
                continue
            record = records_by_chunk_id.get(str(metadata.get("chunk_id") or ""))
            if record is not None:
                if isinstance(record.chunk_metadata, dict):
                    metadata.update(record.chunk_metadata)
                metadata["start_char_pos"] = record.start_char_pos
                metadata["end_char_pos"] = record.end_char_pos
            metadata["content_hash"] = content_hash_by_file_id.get(str(metadata.get("file_id") or ""))
    ...
    async def index_file(
        ...
        try:
            ...
            from yuxi.knowledge.chunking.pdf_locator import attach_pdf_page_ranges

            chunks = await asyncio.to_thread(attach_pdf_page_ranges, chunks, file_meta.get("path"))
            logger.info(
            ...
            academic_metadata = next(
                (
                    metadata.get("paper")
                    for chunk in chunks
                    if isinstance((metadata := chunk.get("chunk_metadata")), dict)
                    and metadata.get("document_type") == "academic_paper"
                    and isinstance(metadata.get("paper"), dict)
                ),
                None,
            )
            if academic_metadata:
                await AcademicPaperRepository().upsert_for_file(
                    kb_id=kb_id,
                    file_id=file_id,
                    data={**academic_metadata, "metadata_error": None},
                )
    ...
    async def update_content(self, kb_id: str, file_ids: list[str], params: dict | None = None) -> list[dict]:
        ...
        for file_id in file_ids:
            ...
            try:
                ...
                chunks = await asyncio.to_thread(attach_pdf_page_ranges, chunks, file_path)
    ...
    async def aquery(self, query_text: str, kb_id: str, agent_call: bool = False, **kwargs) -> list[dict]:
        ...
        strict_research = bool(merged_kwargs.get("strict_research", False))
        ...
        try:
            ...
            if search_mode not in {"vector", "keyword", "hybrid"}:
                if strict_research:
                    raise ValueError(f"不支持的科研检索模式: {search_mode}")
                search_mode = "vector"
            ...
            requested_file_ids = merged_kwargs.get("file_ids")
            if requested_file_ids is not None:
                if not isinstance(requested_file_ids, (list, tuple, set)):
                    raise ValueError("科研检索 file_ids 必须是数组")
                file_expr = self._build_file_ids_expr([str(file_id) for file_id in requested_file_ids])
            if file_expr:
                ...
                if strict_research:
                    ...
                    vector_chunks = [
                        self._build_chunk_from_hit(
                            hit,
                            float(hit.distance or 0.0),
                            include_distances,
                            score_field="vector_score",
                        )
                        for hit in (vector_results[0] if vector_results and vector_results[0] else [])
                    ]
                    bm25_chunks = [
                        self._build_chunk_from_hit(
                            hit,
                        ...
                        for hit in (bm25_results[0] if bm25_results and bm25_results[0] else [])
                    ]
                    retrieved_chunks = self._fuse_research_recall(
                        vector_chunks,
                        bm25_chunks,
            ...
            if use_graph_retrieval:
                graph_chunks = await self._retrieve_graph_chunks(
                    query_text, kb_id, retrieved_chunks, merged_kwargs, strict=strict_research
                )
                if graph_chunks:
            ...
            try:
                ...
                try:
                    ...
                    rerank_scores = await reranker.acompute_score(
                        [query_text, documents_text], normalize=True, strict=strict_research
                    )
                    if len(rerank_scores) != len(retrieved_chunks):
                        raise ValueError(
                            f"Reranker 返回结果数量不匹配: expected={len(retrieved_chunks)}, got={len(rerank_scores)}"
                        )
    ...
    async def _retrieve_graph_chunks(
        ...
        *,
        ...
        try:
            ...
            if not embedding_model_spec:
                if strict:
                    raise ValueError("图谱检索缺少 embedding 模型配置")
                return []
            ...
            allowed_file_ids = query_params.get("file_ids")
            if allowed_file_ids is not None:
                allowed = {str(file_id) for file_id in allowed_file_ids}
                chunks = [chunk for chunk in chunks if str(chunk.file_id) in allowed]
    ...
    def _fuse_research_recall(
        ...
        *,
        ...
        def merge(chunk: dict, rank: int, weight: float, score_field: str, source: str) -> None:
            chunk_id = str((chunk.get("metadata") or {}).get("chunk_id") or "")
            if not chunk_id:
                raise ValueError("严格科研召回结果缺少 chunk_id")
            entry = fused.setdefault(
                chunk_id,
                {**chunk, "hybrid_score": 0.0, "fusion_sources": []},
            )
            entry[score_field] = float(chunk.get(score_field) or 0.0)
            entry["hybrid_score"] += max(weight, 0.0) / (rrf_k + rank)
            entry["fusion_sources"].append(source)

        for rank, chunk in enumerate(vector_chunks, start=1):
            merge(chunk, rank, vector_weight, "vector_score", "vector")
        for rank, chunk in enumerate(bm25_chunks, start=1):
            merge(chunk, rank, bm25_weight, "bm25_score", "bm25")
