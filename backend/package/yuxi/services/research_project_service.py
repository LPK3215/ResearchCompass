"""ResearchCompass 研究项目治理服务。

本模块是本仓库作者在开源智能体框架 Yuxi 之上实现的研究项目主流程：项目生命周期、
状态机、研究成果归集（论文/检索/综述/分析/消融实验）、活动日志与可见性校验。
知识库访问控制、持久化与执行计划摘要由 Yuxi 仓库与计划工具提供；本模块定义研究
项目的状态约束、资产快照语义与跨成果类型的归集规则。
"""
