...
class PostgresManager(metaclass=SingletonMeta):
    ...
    async def ensure_knowledge_schema(self):
        ...
        stmts = [
            ...
            """CREATE TABLE IF NOT EXISTS research_project_risks (
                id SERIAL PRIMARY KEY,
                ...
                CONSTRAINT ck_research_project_risks_severity CHECK (severity IN ('low', 'medium', 'high', 'critical')),
                CONSTRAINT ck_research_project_risks_status CHECK (status IN ('open', 'mitigating', 'accepted', 'resolved', 'closed'))
            )""",
            "CREATE INDEX IF NOT EXISTS ix_research_project_risks_project_status ON research_project_risks (project_id, status)",
            "CREATE INDEX IF NOT EXISTS ix_research_project_risks_owner ON research_project_risks (owner_uid)",
            """CREATE TABLE IF NOT EXISTS research_project_risk_activities (
                id SERIAL PRIMARY KEY,
                activity_id VARCHAR(64) UNIQUE NOT NULL,
                risk_id VARCHAR(64) NOT NULL REFERENCES research_project_risks(risk_id) ON DELETE CASCADE,
                project_id VARCHAR(64) NOT NULL REFERENCES research_projects(project_id) ON DELETE CASCADE,
            ...
            "CREATE INDEX IF NOT EXISTS ix_research_project_risk_activities_risk_created ON research_project_risk_activities (risk_id, created_at)",
            ...
            (
                "ALTER TABLE IF EXISTS academic_papers ADD COLUMN IF NOT EXISTS "
            ...
            BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint WHERE conname = 'uq_academic_papers_kb_paper'
                ) THEN
                    ALTER TABLE academic_papers
                    ADD CONSTRAINT uq_academic_papers_kb_paper UNIQUE (kb_id, paper_id);
                END IF;
            END $$
            """,
            "ALTER TABLE IF EXISTS evaluation_datasets ADD COLUMN IF NOT EXISTS created_by VARCHAR(64)",
            "ALTER TABLE IF EXISTS research_user_studies ADD COLUMN IF NOT EXISTS consent_text TEXT",
            "ALTER TABLE IF EXISTS research_user_studies ADD COLUMN IF NOT EXISTS status VARCHAR(32)",
            "ALTER TABLE IF EXISTS research_user_studies ADD COLUMN IF NOT EXISTS closed_at TIMESTAMPTZ",
            "UPDATE research_user_studies SET consent_text = '' WHERE consent_text IS NULL",
            "ALTER TABLE IF EXISTS research_user_studies ALTER COLUMN consent_text SET NOT NULL",
            "UPDATE research_user_studies SET status = 'open' WHERE status IS NULL",
            "ALTER TABLE IF EXISTS research_user_studies ALTER COLUMN status SET NOT NULL",
            ...
            "ALTER TABLE IF EXISTS evaluation_runs ADD COLUMN IF NOT EXISTS experiment_id VARCHAR(64)",
            "ALTER TABLE IF EXISTS evaluation_runs ADD COLUMN IF NOT EXISTS variant_id VARCHAR(64)",
            ...
            "ALTER TABLE IF EXISTS evaluation_run_items ADD COLUMN IF NOT EXISTS gold_document_hashes JSONB",
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS research_user_studies (
                id SERIAL PRIMARY KEY,
                study_id VARCHAR(64) NOT NULL UNIQUE,
                kb_id VARCHAR(80) NOT NULL REFERENCES knowledge_bases(kb_id) ON DELETE CASCADE,
                name VARCHAR(255) NOT NULL,
                description TEXT,
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS academic_paper_analysis_evaluations (
                id SERIAL PRIMARY KEY,
                ...
                rubric_version VARCHAR(32) NOT NULL DEFAULT 'analysis-blind-v1',
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS academic_paper_analysis_evaluation_items (
                id SERIAL PRIMARY KEY,
                item_id VARCHAR(64) NOT NULL UNIQUE,
                evaluation_id VARCHAR(64) NOT NULL REFERENCES academic_paper_analysis_evaluations(evaluation_id)
                    ON DELETE CASCADE,
                academic_paper_id INTEGER NOT NULL REFERENCES academic_papers(id) ON DELETE CASCADE,
                item_index INTEGER NOT NULL,
                single_run_id VARCHAR(64),
                multi_run_id VARCHAR(64),
                blind_assignment JSONB NOT NULL,
                status VARCHAR(32) NOT NULL DEFAULT 'pending',
                error_message TEXT,
                created_at TIMESTAMPTZ DEFAULT NOW(),
                completed_at TIMESTAMPTZ,
                CONSTRAINT uq_academic_paper_analysis_evaluation_paper UNIQUE (evaluation_id, academic_paper_id)
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS academic_paper_analysis_evaluation_scores (
                id SERIAL PRIMARY KEY,
                score_id VARCHAR(64) NOT NULL UNIQUE,
                evaluation_id VARCHAR(64) NOT NULL REFERENCES academic_paper_analysis_evaluations(evaluation_id)
                    ON DELETE CASCADE,
                item_id VARCHAR(64) NOT NULL REFERENCES academic_paper_analysis_evaluation_items(item_id)
                    ON DELETE CASCADE,
                scorer_uid VARCHAR(64) NOT NULL,
                blind_scores JSONB NOT NULL,
                notes TEXT,
                created_at TIMESTAMPTZ DEFAULT NOW(),
                submitted_at TIMESTAMPTZ DEFAULT NOW(),
                CONSTRAINT uq_academic_paper_analysis_evaluation_scorer UNIQUE (item_id, scorer_uid)
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS research_user_study_invites (
                id SERIAL PRIMARY KEY,
                invite_id VARCHAR(64) NOT NULL UNIQUE,
                study_id VARCHAR(64) NOT NULL REFERENCES research_user_studies(study_id) ON DELETE CASCADE,
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS research_user_study_responses (
                id SERIAL PRIMARY KEY,
                response_id VARCHAR(64) NOT NULL UNIQUE,
                study_id VARCHAR(64) NOT NULL REFERENCES research_user_studies(study_id) ON DELETE CASCADE,
                invite_id VARCHAR(64) NOT NULL UNIQUE
                    REFERENCES research_user_study_invites(invite_id) ON DELETE RESTRICT,
                research_stage VARCHAR(32) NOT NULL,
                research_experience VARCHAR(32) NOT NULL,
                task_scores JSONB NOT NULL,
                sus_scores JSONB NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS evaluation_experiments (
                ...
                name VARCHAR(255) NOT NULL,
                description TEXT,
                source_kb_id VARCHAR(80) NOT NULL REFERENCES knowledge_bases(kb_id) ON DELETE CASCADE,
                dataset_id VARCHAR(64) NOT NULL REFERENCES evaluation_datasets(dataset_id) ON DELETE RESTRICT,
                status VARCHAR(32) NOT NULL DEFAULT 'queued',
                baseline_variant_id VARCHAR(64),
            ...
            CREATE TABLE IF NOT EXISTS evaluation_experiment_variants (
                ...
                experiment_id VARCHAR(64) NOT NULL REFERENCES evaluation_experiments(experiment_id) ON DELETE CASCADE,
                ...
                run_id VARCHAR(64) UNIQUE REFERENCES evaluation_runs(run_id) ON DELETE SET NULL,
                status VARCHAR(32) NOT NULL DEFAULT 'queued',
                metrics JSONB,
                overall_score DOUBLE PRECISION,
                error_message TEXT,
                started_at TIMESTAMPTZ,
                completed_at TIMESTAMPTZ,
                CONSTRAINT uq_evaluation_experiment_variant_index UNIQUE (experiment_id, variant_index)
            )
            """,
            """
            ...
            (
                "CREATE INDEX IF NOT EXISTS ix_research_user_studies_kb_created "
                "ON research_user_studies(kb_id, created_at DESC)"
            ),
            "CREATE INDEX IF NOT EXISTS ix_research_user_studies_status ON research_user_studies(status)",
            (
                "CREATE INDEX IF NOT EXISTS ix_academic_paper_analysis_evaluations_kb_created "
                "ON academic_paper_analysis_evaluations(kb_id, created_at DESC)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS ix_academic_paper_analysis_evaluation_items_evaluation "
                "ON academic_paper_analysis_evaluation_items(evaluation_id, item_index)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS ix_academic_paper_analysis_evaluation_scores_evaluation "
                "ON academic_paper_analysis_evaluation_scores(evaluation_id, submitted_at DESC)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS ix_research_user_study_invites_study_status "
                "ON research_user_study_invites(study_id, status)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS ix_research_user_study_responses_study_submitted "
                "ON research_user_study_responses(study_id, submitted_at DESC)"
            ),
            (
            ...
            "CREATE INDEX IF NOT EXISTS ix_evaluation_runs_experiment_id ON evaluation_runs(experiment_id)",
            "CREATE INDEX IF NOT EXISTS ix_evaluation_runs_variant_id ON evaluation_runs(variant_id)",
            "CREATE INDEX IF NOT EXISTS ix_evaluation_run_items_run_index ON evaluation_run_items(run_id, item_index)",
            "CREATE INDEX IF NOT EXISTS ix_evaluation_experiments_status ON evaluation_experiments(status)",
            (
                "CREATE INDEX IF NOT EXISTS ix_evaluation_experiments_created_at "
                "ON evaluation_experiments(created_at DESC)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS ix_evaluation_experiments_source_kb_id "
                "ON evaluation_experiments(source_kb_id)"
            ),
            ("CREATE INDEX IF NOT EXISTS ix_evaluation_experiments_dataset_id ON evaluation_experiments(dataset_id)"),
            (
                "CREATE INDEX IF NOT EXISTS ix_evaluation_experiment_variants_experiment_id "
                "ON evaluation_experiment_variants(experiment_id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS ix_evaluation_experiment_variants_kb_id "
                "ON evaluation_experiment_variants(kb_id)"
            ),
