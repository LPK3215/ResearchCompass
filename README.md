...
- 对话式研究 Copilot：完整模式登录后直接进入 ResearchCompass，AI 研究助手自动获得当前知识库、研究项目、工作区和选中对象上下文，并通过受权限与审批约束的领域工具执行项目规划、论文检索、证据综述和成果归集；所有结果继续写回结构化工作台。Lite 模式不加载研究依赖，默认进入通用助手。
