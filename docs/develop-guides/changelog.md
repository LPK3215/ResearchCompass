...
- 对话页升级为科研主交互入口：默认入口从 /research 改为 /agent（科研助手），底层默认 agent 从 default-chatbot 替换为 research-copilot，复用框架原生对话页的工具调用展示、文档加载和流式渲染能力；用户进入即自动选中第一个知识库并创建研究会话线程，无需手动切换。/research 工作台保留全部现有功能作为结果检视器和备用入口，两个入口功能一致、均可独立使用。default-chatbot 仍在智能体切换列表中可选。导航"通用助手"更名为"科研助手"以反映新定位。
