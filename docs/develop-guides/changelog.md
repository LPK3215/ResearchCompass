...
- 完成依赖漏洞升级与可观测性降噪（R-07/R-08/E-07）：将 `pypdf` 约束从 `>=6.13.3` 升级到 `>=6.14.2`（修复 4 项 DoS/无限循环漏洞），新增 `mcp>=1.28.1,<2.0.0` 约束（修复 WebSocket transport 缺 Host/Origin 校验漏洞，同时避免 mcp 2.0.0 的 `RequestContext` 破坏性 `ImportError`），新增 `pymilvus<3.1` 约束（确保在 ORM API 移除前完成 MilvusClient 迁移）；通过 `uv lock` 刷新拉取 pillow/pyasn1/json-repair/langsmith/pydantic-settings 等传递依赖的安全版本，`pip-audit` 从 34 项降至 3 项剩余（nltk 3.9.4 路径遍历无修复版本、setuptools 81.0.0 构建时 Unicode 规范化绕过 2 项）。Neo4j 驱动配置新增 `notifications_min_severity='ERROR'`，过滤空图状态下 label/property key 不存在的 WARNING 通知，真实存储故障仍以 ERROR 可见（R-08 部分）。前端 `ResearchCompassView.vue` 的 `.research-content` 新增 `overflow-x: hidden`，修复"用户评测"标签被右侧面板遮挡的问题。全量回归通过：后端 unit 1505 passed/1 skipped、integration 187 passed/4 skipped，前端测试、ESLint、生产构建均通过。
