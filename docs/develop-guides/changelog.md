...
- 智能检索新增严格图谱模式前置校验：选中"严格图谱"检索模式时自动查询引用图谱状态，图谱未同步或论文/引用关系为空时显示黄色警告并拦截检索提交，引导用户先同步图谱或切换为"本地混合"模式，避免提交后才收到 `graph_not_ready` 错误。智能体编辑弹窗优化关闭交互：取消按钮在图标上传期间同步禁用，遮罩层点击在保存和上传期间禁用，避免操作不一致。主题初始化支持 `prefers-color-scheme`：未保存主题偏好时自动跟随系统暗色模式设置，登录页也能正确适配暗色主题。Semantic Scholar 客户端默认携带 `User-Agent` 请求头，符合 API 最佳实践。
